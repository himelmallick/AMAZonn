...
# m1 <- zeroinfl(Dvisits ~ . | ., data = new.dat, weights = NULL, dist = "negbin", model = T, y = T, x = F)
# summary(m1)
...
fit.lasso = zipath(Dvisits ~ . | ., data = new.dat, family = "negbin",
...
fit.Alasso = ALasso(Dvisits ~ . | ., data = new.dat, family = "negbin",
                       nlambda = 100, lambda.zero.min.ratio = 0.001, maxit.em = 300,
                       maxit.theta = 25, theta.fixed = FALSE, trace = FALSE,
                       penalty = "enet", rescale = FALSE)
...
fit.scad = zipath(Dvisits ~ . | ., data = new.dat, family = "negbin",
...
fit.zonn = AMAZonn(Dvisits ~ . | ., data = new.dat, family = "negbin",
                       nlambda = 100, lambda.zero.min.ratio = 0.001, maxit.em = 300,
                       maxit.theta = 25, theta.fixed = FALSE, trace = FALSE,
                       penalty = "enet", rescale = FALSE)
...
write.csv(tab,"racd_Results_ZINB.csv")